...
private:
	CPolopointsAddress *addr;
public:
	CDacrsAddressVisitor(CPolopointsAddress *addrIn) :
			addr(addrIn) {
	}
...
bool CPolopointsAddress::Set(const CKeyID &id) {
...
bool CPolopointsAddress::Set(const CTxDestination &dest) {
	return boost::apply_visitor(CDacrsAddressVisitor(this), dest);
}

bool CPolopointsAddress::IsValid() const {
...
CTxDestination CPolopointsAddress::Get() const {
	if (!IsValid())
...
bool CPolopointsAddress::GetKeyID(CKeyID &keyID) const {
	uint160 id;
...
//bool CPolopointsAddress::GetRegID(CRegID &Regid) const {
//
////	vector<unsigned char> vid;
////	vid.push_back(CBaseParams::ACC_ADDRESS);
...
bool CPolopointsAddress::IsScript() const {
	return IsValid() && vchVersion == SysCfg().Base58Prefix(CBaseParams::SCRIPT_ADDRESS);
}

void CPolopointsSecret::SetKey(const CKey& vchSecret) {
...
CKey CPolopointsSecret::GetKey() {
	CKey ret;
	ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
	return ret;
}

bool CPolopointsSecret::IsValid() const {
	bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
	bool fCorrectVersion = vchVersion == SysCfg().Base58Prefix(CBaseParams::SECRET_KEY);
	return fExpectedFormat && fCorrectVersion;
}

bool CPolopointsSecret::SetString(const char* pszSecret) {
	return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CPolopointsSecret::SetString(const string& strSecret) {
