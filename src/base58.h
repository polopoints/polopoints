...
//
#ifndef POLOPOINTS_BASE58_H
#define POLOPOINTS_BASE58_H
...
/** base58-encoded Polopoints addresses.
 * Public-key-hash-addresses have version 0 (or 111 testnet).
 * The data vector contains RIPEMD160(SHA256(pubkey)), where pubkey is the serialized public key.
 * Script-hash-addresses have version 5 (or 196 testnet).
 * The data vector contains RIPEMD160(SHA256(cscript)), where cscript is the serialized redemption script.
 */
class CPolopointsAddress : public CBase58Data {
public:
    ...
    CPolopointsAddress() {}
    CPolopointsAddress(const CTxDestination &dest) { Set(dest); }
    CPolopointsAddress(const string& strAddress) { SetString(strAddress); }
    CPolopointsAddress(const char* pszAddress) { SetString(pszAddress); }
 ...
 */
class CPolopointsSecret : public CBase58Data
{
public:
    ...
    CPolopointsSecret(const CKey& vchSecret) { SetKey(vchSecret); }
    CPolopointsSecret() {}
};

template<typename K, int Size, CBaseParams::Base58Type Type> class CPolopointsExtKeyBase : public CBase58Data
{
public:
    ...
    CPolopointsExtKeyBase(const K &key) {
        SetKey(key);
    }

    CPolopointsExtKeyBase() {}
};

typedef CPolopointsExtKeyBase<CExtKey, 74, CBaseParams::EXT_SECRET_KEY> CPolopointsExtKey;
typedef CPolopointsExtKeyBase<CExtPubKey, 74, CBaseParams::EXT_PUBLIC_KEY> CPolopointsExtPubKey;

#endif // POLOPOINTS_BASE58_H
