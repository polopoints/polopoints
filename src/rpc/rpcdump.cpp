...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"Polopointsaddress\"   (string, required) The Polopoints address for the private key\n"
            "\nResult:\n"
    ...
    CPolopointsAddress address;
    if (!address.SetString(strAddress))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Polopoints address");
    CKeyID keyID;
    ...
    if (!pwalletMain->GetKey(keyID, minerkey,true))
           throw JSONRPCError(RPC_WALLET_ERROR, "Private key for address " + strAddress + " is not known");
    Object reply;
    	reply.push_back(Pair("privkey", CPolopointsSecret(vchSecret).ToString()));
    if(minerkey.ToString() != vchSecret.ToString())
    	reply.push_back(Pair("minerkey", CPolopointsSecret(minerkey).ToString()));
    else
...
Value dumpwallet(const Array& params, bool fHelp) {
	...
	Object reply;
	reply.push_back(Pair("created by Polopoints", CLIENT_BUILD + CLIENT_DATE));
