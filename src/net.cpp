...
// Copyright (c) 2009-2014 The Polopoints developers
...
limitedmap<CInv, int64_t> mapAlreadyAskedFor(MAX_INV_SZ);   //存放发送请求的 MSG_TX ，int64_t是 the earliest time the request can be sent，收到对应交易执行erase
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Polopoints users should be well used to small numbers with many decimal places by now :)
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Polopoints Core is probably already running."), addrBind.ToString());
        else
