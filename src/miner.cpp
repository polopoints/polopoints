...
// Copyright (c) 2014-2015 The Polopoints developers
...
//	LogPrint("INFO","proof-of-work found  \n  hash: %s  \ntarget: %s\n", hash.GetHex(), hashTarget.GetHex());
	...
	// Found a solution
	{
		...
		if (pblock->GetHashPrevBlock() != chainActive.Tip()->GetBlockHash())
			return ERRORMSG("PolopointsMiner : generated block is stale");
...
//		}
		...
		if (!ProcessBlock(state, NULL, pblock))
			return ERRORMSG("PolopointsMiner : ProcessBlock, block not accepted");
	}
...
void static PolopointsMiner(CWallet *pwallet,int targetConter) {
	LogPrint("INFO","Miner started\n");

	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("Polopoints-miner");
	...
	if (!CheckIsHaveMinerKey()) {
			LogPrint("INFO", "PolopointsMiner  terminated\n");
...
void GeneratePolopointsBlock(bool fGenerate, CWallet* pwallet, int targetHigh) {
...
//		assert(0);
	...
	minerThreads->create_thread(boost::bind(&PolopointsMiner, pwallet,targetHigh));
