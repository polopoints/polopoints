...
int LogPrintStr(const char* category, const string &str) {
	...
	if (SysCfg().IsPrintToFile()) {
		...
		pathDebug = GetDataDir() / file;   // /home/share/bille/POLOPOINTS_test/regtest/INFO.log
...
#else
	const char* pszModule = "Polopoints";
#endif
	if (pex)
		return strprintf(
...
boost::filesystem::path GetDefaultDataDir() {
	...
	// Windows < Vista: C:\Documents and Settings\Username\Application Data\Polopoints
	// Windows >= Vista: C:\Users\Username\AppData\Roaming\Polopoints
	// Mac: ~/Library/Application Support/Polopoints
	// Unix: ~/.Polopoints
#ifdef WIN32
	// Windows
	return GetSpecialFolderPath(CSIDL_APPDATA) / "Polopoints";
#else
	fs::path pathRet;
...
boost::filesystem::path GetConfigFile() {
	boost::filesystem::path pathConfigFile(CBaseParams::GetArg("-conf", "polopoints.conf"));
			...
			if (!fDone) {
				...
				if (!fMatch) {
					...
					string strMessage = _("Warning: Please check that your computer's date and time "
							"are correct! If your clock is wrong Polopoints will not work properly.");
